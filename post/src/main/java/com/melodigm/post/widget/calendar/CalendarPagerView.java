...
import static com.melodigm.post.widget.calendar.MaterialCalendarView.SHOW_DEFAULTS;
...
abstract class CalendarPagerView extends ViewGroup implements View.OnClickListener {
    ...
    // YCLOVE : MaterialCalendarView - 요일을 빌드한다.
    ...
    protected void addDayView(Collection<DayView> dayViews, Calendar calendar) {
        ...
        param.leftMargin = size;
        param.rightMargin = size;

        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(size * 3, size * 3);
        params.leftMargin = size;
        params.rightMargin = size;

        dayView.setLayoutParams(params);
        addView(dayView, param);
        calendar.add(DATE, 1);
    }
    ...
    public void setWeekDayTextAppearance(int taId) {
        // YCLOVE : MaterialCalendarView - 요일에 스타일을 제거한다.
