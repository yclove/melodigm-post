 ...
 */
public class MusicServiceForCache extends Service implements IOnHandlerMessage, AudioManager.OnAudioFocusChangeListener, CacheListener, MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener, MediaPlayer.OnCompletionListener {
    ...
    public void onCreate() {
        ...
        LogUtil.e(String.format(Locale.US, "♬ MusicService 생성 ▶ SONG_NUMBER(%d), MPS_REPEAT(%d), MPS_RANDOM(%d)", PlayerConstants.SONG_NUMBER, PlayerConstants.MPS_REPEAT, PlayerConstants.MPS_RANDOM));
    ...
    public void onCacheAvailable(File file, String url, int percentsAvailable) {
        LogUtil.e(String.format(Locale.US, "onCacheAvailable : (%d) %s(%s), url(%s)", percentsAvailable, file, CastUtil.getFileSize(file.length()), url));
    }
    ...
    public void onDestroy() {
        ...
        stopTimer();
        PlayerConstants.SONG_PAUSED = true;
        BaseApplication.getProxy(mContext).unregisterCacheListener(this);
    }

    private void playMusic() {
        LogUtil.e("♬ MusicService ▶ playMusic");
        ...
        String url;
        if (PlayerConstants.SONG_RADIO)
            url = Constants.API_POST + "mus0401v1.post?REQ_DATA={'POI':'" + PlayerConstants.SONGS_LIST.get(PlayerConstants.SONG_NUMBER).getPOI() + "'}";
        else
            url = Constants.API_POST + "mus0401v1.post?REQ_DATA={'SSI':'" + PlayerConstants.SONGS_LIST.get(PlayerConstants.SONG_NUMBER).getSSI() + "','TYPE':'" + mFormat + "'}";
